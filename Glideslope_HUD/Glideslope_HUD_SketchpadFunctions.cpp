// ==========================================================================
//
//	Glideslope_HUD (Sketchpad Helper Functions for MFD Display Update)
//	=================================================================
//
...
int Glideslope_HUD::_Line(const int row) {  // row is 0-24, for 24 rows. e.g. Line(12)
  int ret;
  ret = (int)((H - (int)(ch / 4)) * row / 25) + (int)(ch / 4);
  return ret;
};

int Glideslope_HUD::_Col(const int pos) {  // pos is 0-5, for 6 columns. Eg Col(3) for middle
  int ret = (int)((W - (int)(cw / 2)) * pos / 6) + int(cw / 2);
  return ret;
};

int Glideslope_HUD::_Col2(const int pos) {  // pos is 0-11, for 12 columns. Eg Col(6) for middle
...
void Glideslope_HUD::skpFormatText(const int col, const int line, const char* fmt, ...) {
...
void Glideslope_HUD::skpFmtColText(const int col, const int line, const bool test, const DWORD truecol, const DWORD falsecol, const char* fmt, ...) {
...
void Glideslope_HUD::skpColor(const DWORD col) {
  LC->skp->SetTextColor(col);
}

void Glideslope_HUD::skpFmtEngText(const int col, const int line, const char* fmt, const char* sfx, const double val, const int dloB) {
  LC->skpColPix = _Col(col);
  LC->skpLinePix = _Line(line);
...
void Glideslope_HUD::skpFmtEngText(const int col, const int line, const char* fmt, const unsigned char* sfx, const double val, const int dloB) {
  LC->skpColPix = _Col(col);
  LC->skpLinePix = _Line(line);
  char engUnit[12] = "pnum kMGTPE";
  double cnvVal = val;
...
void Glideslope_HUD::skpTitle(const char *title) {
  Title(LC->skp, title);
...
bool Glideslope_HUD::DisplayMessageMode() {
  skpTitle("Glideslope_HUD MFD");
  showMessage();
  return true;
};
...
void Glideslope_HUD::showMessage() {
